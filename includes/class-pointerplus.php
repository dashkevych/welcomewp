...
class PointerPlus {
	...
	function admin_enqueue_assets() {
		...
		wp_enqueue_style(
			$this->prefix,
			WELCOMEWP_PLUGIN_URL . "assets/css/admin/pointerplus{$suffix}.css",
			array( 'wp-pointer' )
		);

		wp_enqueue_script(
			$this->prefix,
			WELCOMEWP_PLUGIN_URL . 'assets/js/admin/pointerplus.js?var=' . str_replace( '-', '_', $this->prefix ) . '_pointerplus',
			array( 'wp-pointer' )
		);

		wp_localize_script(
			$this->prefix,
			str_replace( '-', '_', $this->prefix ) . '_pointerplus',
			apply_filters( $this->prefix . '_pointerplus_js_vars', $this->pointers )
		);
	}
